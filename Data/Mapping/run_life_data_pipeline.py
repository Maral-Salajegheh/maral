...
Run the Life data-preparation steps in order and stop on the first failure.
    ...
    2. 00_map_mid_to_adb.py  MasterIndex -> Analyse-DB mapping and C6 check

Publication to PROC_LIFE_MID_ADB is deliberately not run here. Read the C6
check first, then publish with:
...
def run_all(schema: str, assume_yes: bool) -> None:
    ...
    print("Review the C6 check, then publish with "
