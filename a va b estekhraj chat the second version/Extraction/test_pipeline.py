...
def response(keys):
    ...
    return {"document_kind": "reisepass", "type_evidence": "PASSPORT",
            "identity": {"issuing_state": None, "holder_name": None, "birth_date": None},
...
class PipelineTests(unittest.TestCase):
    def setUp(self):
        self.llm = Mock()
        self.llm.read.side_effect = lambda path, keys: {"documents": [response(keys)]}
    ...
    def test_mrz_success_requests_card_fields_for_association(self):
    ...
    def test_llm_failure_preserves_mrz(self):
        ...
        self.assertEqual(pending[0]["fields"]["ausweisnummer"]["value"], "L898902C3")
        self.assertTrue(pending[0]["association_unresolved"])
    ...
    def test_strict_mrz_and_llm_parsing(self):
        ...
        self.assertEqual(decode(json.dumps({"documents": [answer]}), FIELDS)["documents"][0]["document_kind"], "reisepass")
        del answer["fields"]["geburtsort"]
        with self.assertRaises(ValueError):
            decode({"documents": [answer]}, FIELDS)
