...
def main() -> None:
    ...
    for key, record in sorted(records.items()):
        ...
        if (
            ...
            and previous.get("ausweistyp_version") == config.AUSWEISTYP_VERSION
        ):
            continue
        if record.get("status") != "success" or not record.get("checks_valid"):
            ...
            continue
        output = {**record, "ausweistyp": map_ausweistyp(str(record.get("document_code") or "")), "ausweistyp_version": config.AUSWEISTYP_VERSION, "status": "success", "needs_human_review": False, "error": "", "processed_at_utc": utc_now_iso()}
