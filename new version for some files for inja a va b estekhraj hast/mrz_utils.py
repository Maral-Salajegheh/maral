...
# Ways to fit a short line to its canonical width. OCR under-reads long filler runs, so a
# line can arrive short with its final check digit already attached; padding must then go
# before that digit, not after it, or the digit lands in the wrong position.
def fit_line(line: str, width: int) -> list[tuple[str, int]]:
    delta = len(line) - width
    if delta >= 0:
        return [(line[:width], delta)]
    fits = [(line.ljust(width, "<"), delta)]
    if line and line[-1] != "<":
        fits.append((line[:-1].ljust(width - 1, "<") + line[-1], delta))
    return fits
...
def repair_filler_zones(lines: list[str], fmt: str) -> list[str]:
    ...
    for line_index, start, end in FILLER_ZONES[fmt]:
        ...
        if len(zone) >= 4 and zone.count("<") / len(zone) >= config.MRZ_FILLER_ZONE_RATIO:
...
def build_variants(name: str, width: int, window: list[str]) -> list[dict[str, Any]]:
    if any(abs(len(item) - width) > config.MRZ_WIDTH_TOLERANCE for item in window):
        return []
    width_error = sum(abs(len(item) - width) for item in window)
    variants = []
    for combo in product(*[fit_line(item, width) for item in window]):
        variants.extend(zone_variants(name, [item[0] for item in combo], [item[1] for item in combo], width_error))
    return variants


# Raw and filler-zone-repaired forms of one fitted line set.
def zone_variants(name: str, lines_out: list[str], deltas: list[int], width_error: int) -> list[dict[str, Any]]:
    variants = [{"format": name, "lines": lines_out, "length_deltas": deltas, "width_error": width_error}]
...
def parse_variant(variant: dict[str, Any], lines: list[str]) -> dict[str, Any]:
    ...
    parsed["mrz_parser_version"] = config.MRZ_PARSER_VERSION
    return parsed
...
# The fields a disagreement actually matters for. Two repairs that differ only in filler
# are the same answer; two that differ here are two different documents.
IDENTITY_FIELDS = ("document_number", "date_of_birth", "expiry_date", "nationality", "document_code", "surname", "given_names")


def identity_of(parsed: dict[str, Any]) -> tuple[Any, ...]:
    return tuple(parsed.get(field) for field in IDENTITY_FIELDS)


# The exact character changes a repair made, so review can see what was altered.
def repair_changes(original: list[str], repaired: list[str]) -> list[dict[str, Any]]:
    changes = []
    for line_index, (before, after) in enumerate(zip(original, repaired)):
        for char_index, (old, new) in enumerate(zip(before, after)):
            if old != new:
                changes.append({"line": line_index, "index": char_index, "from": old, "to": new})
    return changes


# Accept a variant only if every check digit passes. A passing checksum is not proof the
# reading is right: several different repairs can each satisfy the arithmetic while giving
# different document numbers, so collect them all and flag disagreement instead of taking
...
def resolve_variant(variant: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if parsed["checks_valid"]:
        parsed.update({"repair_applied": False, "repair_ambiguous": False, "repair_changes": [],
                       "mrz_lines_raw": list(variant["lines"])})
        return parsed
    ...
    for lines, positions in repair_candidates(variant):
        ...
        if candidate["checks_valid"]:
            ...
            candidate["repair_changes"] = repair_changes(variant["lines"], lines)
            accepted.append(candidate)
            if len(accepted) >= config.MRZ_MAX_REPAIRS_EXAMINED:
                break
    if not accepted:
        return None
    identities = {identity_of(item) for item in accepted}
    chosen = min(accepted, key=lambda item: len(item["repair_changes"]))
    chosen.update({"repair_applied": True, "repair_ambiguous": len(identities) > 1,
                   "repair_alternatives": len(identities), "mrz_lines_raw": list(variant["lines"])})
    return chosen
...
def parse_with_repair(text: str) -> dict[str, Any] | None:
    ...
    for variant in candidate_variants(lines):
        ...
        if parsed:
            # Erasing an optional-data zone is destructive even when the checksums then pass.
            parsed["mrz_repair_needs_review"] = bool(parsed.get("repair_ambiguous")) or bool(parsed.get("mrz_filler_repaired"))
            return parsed
    return None
