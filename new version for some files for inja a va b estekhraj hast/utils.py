...
# One physical document. A masterindex_id can hold several PDFs, so pairing a card front
# with its back must not cross the PDF boundary or one person's data lands on another's row.
def document_key(record: dict[str, Any]) -> tuple[str, str]:
    return (str(record.get("masterindex_id") or ""), str(record.get("pdf_path_in_zip") or ""))
